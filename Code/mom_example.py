...
class leadtime_no_negative:
    ...
    def __init__(self, dist_path, ind_demand_path, ind_lead_path, Lead_scale, S, s, max_S,demands, leadtimes,
                 true_arrivals, true_services, num_samples=100000,
                 SIM_TIME=10000):
...
def main():
    ...
    for exmaple in range(0, 1):
        try:
            ...
            samples_path = r'C:\Users\Eshel\workspace\mom_analysis\notebooks'

            fit_arrivals, fit_services = pkl.load(open(os.path.join(samples_path, 'fit_samples_numeric_ggc.pkl'), 'rb'))
            true_arrivals, true_services = pkl.load(open(os.path.join(samples_path, 'true_samples_numeric_ggc.pkl'), 'rb'))
            Lead_scale = 4.24995351729483



            fit_services = fit_services * 3
            true_services  = true_services * 3

            ph_samples = [(fit_arrivals,fit_services),(true_arrivals, true_services)]
            ...
            for jj in range(2):
                ...
                inv_lead = leadtime_no_negative(path_dists, scv_demand, scv_lead, Lead_scale, S, s, max_S,
                                                ph_samples[jj][0], ph_samples[jj][1], true_arrivals, true_services,
            ...
            plt.title(
                f"(S,s) Inventory Distribution (s={s}, S={S}), error in zero={100 * (ys[0][3] - ys[1][3]) / ys[0][3]}")
            plt.show()
